class App {
    ...
    openProjectDetailsModal(projectTitle) {
        ...
        if (projectTitle === 'JusticeGPT') {
            ...
            `;
        } else if (projectTitle === 'SafeDelete Desktop' || projectTitle === 'Raw Disk') {
            contentHtml = `
                <div class="panel" style="display:flex; flex-direction:column; gap:20px; text-align:left;">
                    <div class="badge-tag" style="width:fit-content; border-color:var(--accent-coral); color:var(--accent-coral);">Disk Forensics & Cybersecurity</div>
                    <p style="font-size:1.1rem; line-height:1.7; color:var(--text-primary);">
                        <strong>SafeDelete Desktop</strong> is a premium, native desktop cybersecurity lab and file shredding utility designed to visualize raw physical sector allocation and Shannon entropy patterns.
                    </p>
                    <h3 style="text-transform:uppercase; font-size:16px; color:var(--accent-taupe); border-bottom:1px solid var(--border-low-opacity); padding-bottom:8px; margin-top:10px;">Key Engineering Implementations</h3>
                    <ul style="list-style:disc; padding-left:20px; color:var(--text-secondary); display:flex; flex-direction:column; gap:12px; font-size:14px; line-height:1.6;">
                        <li><strong>Raw Block Mapping:</strong> Developed direct mapping from logical file nodes to physical 512-byte disk blocks, allowing structural discovery.</li>
            ...
            `;
        } else if (projectTitle === 'AuditiAutomate') {
            contentHtml = `
                <div class="panel" style="display:flex; flex-direction:column; gap:20px; text-align:left;">
                    <div class="badge-tag" style="width:fit-content; border-color:var(--accent-coral); color:var(--accent-coral);">Cloud Security / AWS IAM Audit</div>
                    <p style="font-size:1.1rem; line-height:1.7; color:var(--text-primary);">
                        <strong>AuditiAutomate</strong> is a Python-driven AWS security pipeline designed to audit, detect, and automatically prune inactive IAM user credentials, orphaned access keys, and redundant role boundaries.
                    </p>
                    <h3 style="text-transform:uppercase; font-size:16px; color:var(--accent-taupe); border-bottom:1px solid var(--border-low-opacity); padding-bottom:8px; margin-top:5px;">Key Features</h3>
                    <ul style="list-style:disc; padding-left:20px; color:var(--text-secondary); display:flex; flex-direction:column; gap:12px; font-size:14px; line-height:1.6;">
                        <li><strong>AWS Auditing Engine:</strong> Connects securely using Boto3 to read IAM credential reports and identify unused profiles (90+ days idle).</li>
                        <li><strong>Risk Minimization:</strong> Automatically disables unused access keys and restricts over-privileged roles, reducing the network attack surface by 70%.</li>
                        <li><strong>Cost Optimization:</strong> Highlights orphaned or unused AWS resources (ELBs, EBS volumes) associated with dead roles, cutting monthly infrastructure spend.</li>
                    </ul>
                    <h3 style="text-transform:uppercase; font-size:16px; color:var(--accent-taupe); border-bottom:1px solid var(--border-low-opacity); padding-bottom:8px; margin-top:10px;">Security Pipeline Schematic</h3>
                    <div style="background:rgba(0,0,0,0.4); border:1px solid var(--border-color); border-radius:6px; padding:20px; font-family:'Fira Code', monospace; font-size:12px; color:var(--text-primary); line-height:1.6;">
                        <div style="color:var(--text-accent);">[START] Cloud Security Auditing Job</div>
                        <div style="padding-left:15px; color:var(--text-secondary);">│</div>
                        <div style="padding-left:15px; color:var(--text-secondary);">├── [FETCH] Retrieve AWS IAM Credential Report via Boto3 API</div>
                        <div style="padding-left:15px; color:var(--text-secondary);">├── [SCAN] Verify key ages, password rotations, & active roles</div>
                        <div style="padding-left:15px; color:var(--text-secondary);">│   ├── Active Keys: <span style="color:#00FF66;">[OK]</span></div>
                        <div style="padding-left:15px; color:var(--text-secondary);">│   └── Idle Keys (>90 days): <span style="color:var(--accent-coral);">[FLAGGED FOR PRUNING]</span></div>
                        <div style="padding-left:15px; color:var(--text-secondary);">├── [ACTION] Auto-disable flagged keys & restrict over-privileged IAM JSON policy rules</div>
                        <div style="padding-left:15px; color:var(--text-secondary);">└── [LOG] Export compliance metrics JSON to security audit logs</div>
                        <div style="color:var(--text-accent);">[END] Security Auditing Complete // Risk Reduced by 70%</div>
                    </div>
                    <div style="margin-top:15px; display:flex; justify-content:flex-start;">
                        <a href="https://github.com/D5BU/AuditiAutomate" target="_blank" rel="noopener noreferrer" class="btn-pill" style="display:inline-flex; align-items:center; gap:8px; text-decoration:none;">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.166 6.839 9.489.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.464-1.11-1.464-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.579.688.481C19.137 20.162 22 16.418 22 12c0-5.523-4.477-10-10-10z" />
                            </svg>
                            <span>View on GitHub</span>
            ...
            `;
        } else if (projectTitle === 'PhishGuard') {
            contentHtml = `
                <div class="panel" style="display:flex; flex-direction:column; gap:20px; text-align:left;">
                    <div class="badge-tag" style="width:fit-content; border-color:var(--accent-coral); color:var(--accent-coral);">Cybersecurity / Phishing Mitigation</div>
                    <p style="font-size:1.1rem; line-height:1.7; color:var(--text-primary);">
                        <strong>PhishGuard</strong> is a JavaScript/HTML interactive framework designed to educate developers and security teams on detecting email threat signatures, analyzing reputation indicators, and preventing social engineering attacks.
                    </p>
                    <h3 style="text-transform:uppercase; font-size:16px; color:var(--accent-taupe); border-bottom:1px solid var(--border-low-opacity); padding-bottom:8px; margin-top:5px;">Key Features</h3>
                    <ul style="list-style:disc; padding-left:20px; color:var(--text-secondary); display:flex; flex-direction:column; gap:12px; font-size:14px; line-height:1.6;">
                        <li><strong>Header Diagnostics:</strong> Analyzes raw email headers to identify spoofed SPF, DKIM, and DMARC alignments.</li>
                        <li><strong>Link Reputation Scanner:</strong> Inspects embedded URLs against simulated phishing directories and highlights deceptive domain characters (Homograph attacks).</li>
                        <li><strong>Social Engineering Trainer:</strong> Evaluates email urgency, visual layout mismatches, and recipient patterns to score threat probabilities.</li>
                    </ul>
                    <h3 style="text-transform:uppercase; font-size:16px; color:var(--accent-taupe); border-bottom:1px solid var(--border-low-opacity); padding-bottom:8px; margin-top:10px;">Threat Analyzer Checklist (PhishGuard Suite)</h3>
                    <div style="background:rgba(0,0,0,0.4); border:1px solid var(--border-color); border-radius:6px; padding:18px; display:flex; flex-direction:column; gap:12px; font-family:'Fira Code', monospace; font-size:13px;">
                        <div style="display:flex; align-items:center; gap:8px;">
                            <span style="color:#00FF66;">[✓]</span> <span style="color:var(--text-secondary);">Sender Alignment check (SPF matched)</span>
                        </div>
                        <div style="display:flex; align-items:center; gap:8px;">
                            <span style="color:#00FF66;">[✓]</span> <span style="color:var(--text-secondary);">DKIM Cryptographic signature validation</span>
                        </div>
                        <div style="display:flex; align-items:center; gap:8px;">
                            <span style="color:var(--accent-coral);">[✗]</span> <span style="color:var(--text-primary); font-weight:bold;">URL Check: Domain redirects to fake 'g00gle.com' (Homograph spoof)</span>
                        </div>
                        <div style="display:flex; align-items:center; gap:8px;">
                            <span style="color:var(--accent-coral);">[✗]</span> <span style="color:var(--text-primary); font-weight:bold;">Urgency Analysis: Demands immediate credential verification</span>
                        </div>
                    </div>
                    <div style="margin-top:15px; display:flex; justify-content:flex-start;">
                        <a href="https://github.com/D5BU/PhishGuard" target="_blank" rel="noopener noreferrer" class="btn-pill" style="display:inline-flex; align-items:center; gap:8px; text-decoration:none;">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.166 6.839 9.489.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.464-1.11-1.464-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.579.688.481C19.137 20.162 22 16.418 22 12c0-5.523-4.477-10-10-10z" />
                            </svg>
                            <span>View on GitHub</span>
...
=============================================<br>
            ...
            contentHtml = `
                <div class="panel" style="display:flex; flex-direction:column; gap:20px; text-align:left;">
                    <div class="badge-tag" style="width:fit-content; border-color:var(--text-accent); color:var(--text-accent);">Project Overview</div>
                    <p style="font-size:1.1rem; line-height:1.7; color:var(--text-primary);">
                        Detailed specifications and engineering case studies for <strong>${projectTitle}</strong> are currently under active curation.
                    </p>
                    <div style="background:rgba(255,255,255,0.03); border:1px dashed var(--border-color); border-radius:6px; padding:15px; text-align:center;">
                        <p style="font-size:14px; color:var(--text-secondary); margin:0;">
                            Codebase architecture and execution logs can be explored directly on Shubham's GitHub profile.
                        </p>
                    </div>
                    <div style="margin-top:10px; display:flex; gap:12px; flex-wrap:wrap;">
                        <a href="https://github.com/D5BU" target="_blank" rel="noopener noreferrer" class="btn-pill" style="display:inline-flex; align-items:center; gap:8px; text-decoration:none;">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.166 6.839 9.489.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.464-1.11-1.464-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.579.688.481C19.137 20.162 22 16.418 22 12c0-5.523-4.477-10-10-10z" />
                            </svg>
